...
router.get("/:username", check_authentication, check_authorization(constants.USER_PERMISSION), async function (req, res, next) {
    try {
        let cart = await cartController.GetCartByUsername(req.query.username);
        CreateSuccessRes(res, cart, 200);
    } catch (error) {
        next(error);
    }
...
router.get("/me/cart", check_authentication, check_authorization(constants.USER_PERMISSION), async function (req, res, next) {
    try {
